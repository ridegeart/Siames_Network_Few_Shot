...
from loss import mixed_loss,focal_loss,l_softmax_loss,contrastive_loss
...
for i in range(1,500000):
  ...
  #history = siamese.siamese_net.fit(x_train, y_train_0, batch_size=batch_size, validation_data= (x_val,y_val_0), callbacks=callback, verbose=1)
  history = siamese.siamese_net.fit(x_train, y_train_0, batch_size=batch_size, callbacks=callback, verbose=1)

  train_epoch_loss.append(history.history['loss'])
  train_epoch_acc.append(history.history['accuracy'])
  ...
  if i % loss_every == 0:
    ...
    if val_acc >= best:
      ...
      best_epoch = i
      print("saving")
      siamese.siamese_net.save('./weight/best_convnext_closs.h5')
    
    print('starte picture for test')
    path='./graph_folder/'
    num_epoch=i, 
    epochs = [x for x in range(num_epoch[0])]
    print(len(epochs),len(test_epoch_acc))
    #--LOSS--#
